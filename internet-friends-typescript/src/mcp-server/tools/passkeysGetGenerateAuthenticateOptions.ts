/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { passkeysGetGenerateAuthenticateOptions } from "../../funcs/passkeysGetGenerateAuthenticateOptions.js";
import { formatResult, ToolDefinition } from "../tools.js";

export const tool$passkeysGetGenerateAuthenticateOptions: ToolDefinition = {
  name: "passkeys-get-generate-authenticate-options",
  description: `Generate authentication options for passkey login

Generates WebAuthn authentication options for passkey-based login`,
  tool: async (client, ctx) => {
    const [result, apiCall] = await passkeysGetGenerateAuthenticateOptions(
      client,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    const value = result.value;

    return formatResult(value, apiCall);
  },
};
