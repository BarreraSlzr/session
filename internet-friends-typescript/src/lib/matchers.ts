...
import { InternetFriendsDefaultError } from "../models/errors/internetfriendsdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | InternetFriendsDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | InternetFriendsDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new InternetFriendsDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new InternetFriendsDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
